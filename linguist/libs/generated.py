...
class Generated(object):
    ...
    def is_xcode_project_file(self):
        ...
        """
        return self.ext_name in self.XCODE_PROJECT_EXT_NAMES
    ...
    def is_generated_net_docfile(self):
        ...
        dig into the contents to determine if it's a docfile. Luckily,
        these files are extremely structured, so recognizing them is easy.
    ...
    @property
    def is_generated_postscript(self):
        """
        Internal: Is the blob of PostScript generated?

        PostScript files are often generated by other programs.
        If they tell us so, we can detect them.

        Returns true or false.
        """
        if self.ext_name not in ('.ps', '.eps'):
            return False

        creator = None

        # We analyze the "%%Creator:" comment, which contains the
        # author/generator of the file. If there is one, it should be in
        # one of the first few lines.
        creator_lines = self.lines[0:9]
        for creator_line in creator_lines:
            if creator_line.startswith('%%Creator: '):
        ...
        # Most generators write their version number, while human
        # authors' or companies' names don't contain numbers. So look if the
        # line contains digits. Also look for some special cases
        # without version numbers.
        return (
            any(char.isdigit() for char in creator) or
            'mpage' in creator or
            'draw' in creator or
            'ImageMagick' in creator
        )
    ...
    @property
    def is_composer_lock(self):
        """
        Internal: Is the blob a generated php composer lock file?

        Returns true or false.
        """
        return bool(re.compile('composer.lock').search(self.name))

    @property
    def is_vcr_cassette(self):
        """
        Is the blob a VCR Cassette file?

        Returns true or false
        """
        if self.ext_name != '.yml':
            return False
        if len(self.lines) < 2:
            return False

        # VCR Cassettes have "recorded_with: VCR" in the second last line.
        return "recorded_with: VCR" in self.lines[-2]
